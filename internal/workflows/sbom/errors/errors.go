...
import (
	...
	containererrors "github.com/khulnasoft-lab/container-cli/internal/common/errors"
...
func (ef *SbomErrorFactory) NewBadRequestError(err error) *containererrors.ContainerExtensionError {
	return ef.NewError(
		...
		"SBOM generation failed due to bad input arguments. "+
			"Please make sure you are using the latest version of the Khulnasoft CLI.",
	)
}
...
func (ef *SbomErrorFactory) NewUnauthorizedError(err error) *containererrors.ContainerExtensionError {
	return ef.NewError(
		err,
		"Khulnasoft failed to authenticate you based on your API token. "+
			"Please ensure that you have authenticated by running `vulnmap auth`.",
	)
}
	...
	return ef.NewError(
		...
		fmt.Sprintf(
			"Your account is not authorized to perform this action. "+
				...
				"the organization is entitled to use the Khulnasoft API. (Org ID: %s)",
			orgID,
		),
	)
}
